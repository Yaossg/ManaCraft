        ...
        if(Config.weight > 0)
            GameRegistry.registerWorldGenerator((random, chunkX, chunkZ, world, chunkGenerator, chunkProvider) -> {
                for(int i = 0; i < Config.timesManaOre; ++i)
                    MANA.generate(world, random, randBlockPos(chunkX, chunkZ, random, Config.heightManaOre));
                for(int i = 0; i < Config.timesManaIngotOre; ++i)
                    MANAINGOT.generate(world, random, randBlockPos(chunkX, chunkZ, random, Config.heightManaIngotOre));
                if(random.nextInt(Config.mixtureChance - 1) == 0) {
                    BlockPos genPos = randBlockPos(chunkX, chunkZ, random, Config.mixtureHeight);
